<?php
    ...
    if(isset($_GET['id'])){
        ...
        $check = mysqli_num_rows(select($sql));
        
            if($profile != null && $check >0 ){
                $name = $profile['name'];
            ...
            if(isset($_POST['save'])){
                    if (!empty($name)) {
                        if($check > 0){
                            ...
                            $sql = 'insert into info_user(name, dateofbirth, sex, address, email ,phone,website,github,tw,ig,fb,avatar,id) values("'.$name.'", "'.$dateofbirth.'", "'.$sex.'", "'.$address.'", "'.$email.'",'.$phone.', "'.$website.'", "'.$github.'", "'.$tw.'", "'.$ig.'", "'.$fb.'", "'.$avatar.'", "'.$id.'")';
                        }
                        select($sql);
                        echo "<script>
